...
class PrimaryLayout extends PureComponent<PrimaryLayoutProps> {
  ...
  render() {
    ...
    const newRouteList =
      lang !== 'en'
        ? routeList.map((item: { [x: string]: any; name?: any }) => {
    ...
    const currentRoute = newRouteList.find(
      (      _: { route: any }) => _.route && pathToRegexp(_.route).exec(location.pathname)
    )
    ...
    const menus = newRouteList.filter((_: { menuParentId: string }) => _.menuParentId !== '-1')
        ...
        <Layout>
          ...
          >
            ...
            <GlobalFooter
              ...
              copyright={config.copyright} links={[]}            />
          </div>
        </Layout>
      </Fragment>
    )
